...
* Run that shortcut to get a command prompt. Then run the following
  ...
  directories are): 
  `c:\SRC\configure -prefix c:\INSTALL -platform win32-g++ -shared -opensource -confirm-license -make-tool make -debug-and-release -no-icu -no-openssl -opengl desktop -no-angle -skip qtwebengine -skip qtlocation -nomake examples -nomake tests QMAKE_LFLAGS_CONSOLE+=-static-libstdc++`
* Run `make -j4`
* Run `make install`
