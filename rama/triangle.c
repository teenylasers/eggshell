// Modifications by RLS to clean up this library and its interface:
// * Change all 'unsigned long' to PTRINT, otherwise this wont compile 64 bit.
//   This is because the library stores information in the lower bits of
//   pointers and does frequent casts between PTRINT and pointer types.
// * char * --> const char * for command line arguments.
// * Removed non-ANSI declarations from triangle.h.
// * Removed redundant redeclaration of REAL.
// * Don't #define VOID int, use the proper typedef instead.
...
#define decode(ptr, otri)                                                     \
  (otri).orient = (int) ((PTRINT) (ptr) & (PTRINT) 3l);         \
  (otri).tri = (triangle *)                                                   \
                  ((PTRINT) (ptr) ^ (PTRINT) (otri).orient)
...
#define encode(otri)                                                          \
  (triangle) ((PTRINT) (otri).tri | (PTRINT) (otri).orient)
...
#define infected(otri)                                                        \
  (((PTRINT) (otri).tri[6] & (PTRINT) 2l) != 0l)
...
#define sdecode(sptr, osub)                                                   \
  (osub).ssorient = (int) ((PTRINT) (sptr) & (PTRINT) 1l);      \
  (osub).ss = (subseg *)                                                      \
              ((PTRINT) (sptr) & ~ (PTRINT) 3l)
...
#define sencode(osub)                                                         \
  (subseg) ((PTRINT) (osub).ss | (PTRINT) (osub).ssorient)
...

{
  ...
  printf("triangle x%p with orientation %d:\n", t->tri,
         t->orient);
  ...
  if (printvertex == (vertex) NULL)
    ...
    printf("    Origin[%d] = x%p  (%.12g, %.12g)\n",
           (t->orient + 1) % 3 + 3, printvertex,
           printvertex[0], printvertex[1]);
  dest(*t, printvertex);
  if (printvertex == (vertex) NULL)
    printf("    Dest  [%d] = NULL\n", (t->orient + 2) % 3 + 3);
  else
    printf("    Dest  [%d] = x%p  (%.12g, %.12g)\n",
           (t->orient + 2) % 3 + 3, printvertex,
           printvertex[0], printvertex[1]);
  apex(*t, printvertex);
  if (printvertex == (vertex) NULL)
    printf("    Apex  [%d] = NULL\n", t->orient + 3);
  else
    printf("    Apex  [%d] = x%p  (%.12g, %.12g)\n",
           t->orient + 3, printvertex,
           printvertex[0], printvertex[1]);
...

{
  ...
  printf("subsegment x%p with orientation %d and mark %d:\n",
  ...
  if (printvertex == (vertex) NULL)
    ...
    printf("    Segment origin[%d] = x%p  (%.12g, %.12g)\n",
           4 + s->ssorient, printvertex,
           printvertex[0], printvertex[1]);
  segdest(*s, printvertex);
  if (printvertex == (vertex) NULL)
    printf("    Segment dest  [%d] = NULL\n", 5 - s->ssorient);
  else
    printf("    Segment dest  [%d] = x%p  (%.12g, %.12g)\n",
           5 - s->ssorient, printvertex,
           printvertex[0], printvertex[1]);
}
...

{
  ...
  alignptr = (PTRINT) (pool->nowblock + 1);
  /* Align the item on an `alignbytes'-byte boundary. */
  pool->nextitem = (VOID *)
    (alignptr + (PTRINT) pool->alignbytes -
     (alignptr % (PTRINT) pool->alignbytes));
...

{
  ...
  alignptr = (PTRINT) (pool->pathblock + 1);
...

{
  ...
  alignptr = (PTRINT) m->dummytribase;
  m->dummytri = (triangle *)
    (alignptr + (PTRINT) m->triangles.alignbytes -
     (alignptr % (PTRINT) m->triangles.alignbytes));
  ...
  if (b->usesegments) {
    ...
    alignptr = (PTRINT) m->dummysubbase;
    m->dummysub = (subseg *)
      (alignptr + (PTRINT) m->subsegs.alignbytes -
       (alignptr % (PTRINT) m->subsegs.alignbytes));
...

{
  ...
  alignptr = (PTRINT) (getblock + 1);
  foundvertex = (char *) (alignptr + (PTRINT) m->vertices.alignbytes -
                          (alignptr % (PTRINT) m->vertices.alignbytes));
...

{
  ...
  while (totalsamplesleft > 0) {
    ...
    alignptr = (PTRINT) (sampleblock + 1);
    firsttri = (char *) (alignptr +
                         (PTRINT) m->triangles.alignbytes -
                         (alignptr %
                          (PTRINT) m->triangles.alignbytes));
